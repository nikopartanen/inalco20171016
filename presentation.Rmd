...
- Current state of my PhD (vague deadline is in 2019)
    ...
    - Possibilities for better analysis of syntactic structures
...
leaflet() %>% addTiles() %>% addPolygons(data = st_geometry(kom), stroke = 0, fillOpacity = 0.8)
...
leaflet() %>% addTiles() %>% 
  addPolygons(data = kom, stroke = 0, fillOpacity = 0.8) %>%
...
- Around 10 varieties, each splitting to few subvarieties

---

# IKDP

- The project aimed to produce widely useful dataset from Iźva dialect
...
- Unusually for a documentation project, the annotations were shallow
...
- The avaibality of Komi materials has been growing also otherwise
- Fenno-Ugria collection has produced thousands of digitalized books (picture here)
- Marina Fedina's laboratory in Syktyvkar has been proofreading and digitalizing books and newspapers
- Some online corpora exist as well
- Restructuring old materials is never ending task
...
- Already studied, but there have not been corpus based investigations
...
# Multilingual Bist-parser, SEM

- Developed mainly by KyungTae Lim
